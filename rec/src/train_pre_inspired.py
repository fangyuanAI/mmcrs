...
from model_prompt import MMPrompt_ContextAware


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--seed", type=int, default=42, help="A seed for reproducible training.")
    parser.add_argument("--output_dir", type=str, default='output/rec/src/pre-trained-inspired', help="Where to store the final model.")
    ...
    parser.add_argument("--model", type=str, default='models/DialoGPT-small',help="Path to pretrained model or model identifier from huggingface.co/models.")
...
if __name__ == '__main__':
    ...
    text_simi  = text_sim(pad_entity_id=kg['pad_entity_id'], num_entities=kg['num_entities']).get_entity_ts_info()
    image_simi = image_sim(pad_entity_id=kg['pad_entity_id'], num_entities=kg['num_entities']).get_entity_is_info()
    ...
    data_collator = CRSDataCollator_mm(
        ...
        prompt_tokenizer=text_tokenizer, prompt_max_length=args.prompt_max_length,
        all_items = kg['item_ids'], id_to_idx = text_simi.get('id_to_idx')
    )
    ...
    prompt_encoder = MMPrompt_ContextAware(
        ...
        text_embeds_init=text_simi['text_embeds'],
        image_embeds_init=image_simi['image_embeds']
    ).to(device)
